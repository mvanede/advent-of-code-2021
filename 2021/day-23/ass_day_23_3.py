...
burrow_to_solve = ['.', '.', [['A', True], ['B', True]], '.', [['D', True], ['C', True]], '.', [['C', True],['B', True]], '.', [['A', True],['D', True]], '.', '.']
burrow_to_solve = ['.', '.', [['B', True], ['A', True]], '.', [['C', True], ['D', True]], '.', [['D', True],['A', True]], '.', [['C', True],['B', True]], '.', '.']
ROOMS = [2, 4, 6, 8]
ROOM_SIZE = 2
...
#
# burrow_to_solve = ['.', '.', [['A', True], ['C', True]], '.', [['B', True], ['A', True]], '.', [['C', True], ['B', True]],  '.', '.']
# ROOMS = [2, 4, 6]
# ROOM_SIZE = 2
#
# #
# burrow_to_solve = ['.', '.', [['B', True], ['A', True]], '.', [['A', True],['B', True]], '.', '.']
# ROOM_SIZE = 2
# ROOMS = [2, 4]


# burrow = ['.', '.', [['B', True]], '.',[['A', True]], '.', '.']
# burrow = ['.', ['A', False], [], '.',[], ['B', False], '.']
# ROOM_SIZE = 1
# ROOMS = [2,4]

# burrow_to_solve  = [['A', False], '.', [],'.',[], '.',  ['B', False]]
# ROOM_SIZE = 1
# ROOMS = [2, 4]
...
def solve(_burrow, cost_up_until_here, history=[]):
    if is_solved(_burrow):
        print("SOLVED2! cost=" + str(cost_up_until_here))
    ...
    # todo: dit moet naar boven, voor de cachecheck?
    if is_solved(_burrow):
        SOLVED[cost_up_until_here] = history
        return [(0, True)]
    ...
    # If already more expensive than cheapest solved path, why bother anyways
    if list(SOLVED.keys()) and cost_up_until_here >= min(list(SOLVED.keys())):
        return [(0, False)]
    ...
    for move, cost_of_move in moves:
        ...
        if is_solved(move):
            print("SOLVED! cost=" + str(cost_up_until_here + cost_of_move))
