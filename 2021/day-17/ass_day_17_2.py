...
def calculate_path(start, velocity, target_area):
    ...
    while position[0] <= target_area[1][0] and position[1] >= target_area[1][1]:
        ...
        if position[0] <= target_area[1][0] and position[1] >= target_area[1][1]:
...
def max_height(path):
    return abs(min([h*-1 for w,h in path]))
...
def main():
    ...
    possible_launches = []
    for col in range (0, target_area[1][0] + 1):
        for row in range(target_area[1][1], 200):
            p = calculate_path([0, 0], [col, row], target_area)
            if in_area(p[-1], target_area):
                possible_launches.append([col, row])

    panswer(len(possible_launches))
